...
def collate_fn(examples, tokenizer, pytool):
    ...
    for key in tokenizer.vocab:
        tokenid_pyid[tokenizer.vocab[key]] = pytool.get_pinyin_id(key)  # key = index(vocab.txt)   value = index(py_vocab.txt)
    ...
    # py_labels 是 labels 的拼音序列，pinyin_ids 是错误文本的拼音序列
...
"""
    目前 max_sen_len = 180
    example.source 是错误的文本，target 是正确的文本
"""
...
if __name__ == '__main__':
    x = list(read_train_ds('datas/train.txt'))[0]
    print(type(x))
